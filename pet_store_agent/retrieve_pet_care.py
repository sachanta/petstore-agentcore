...
"""
Amazon Bedrock Knowledge Base retrieval tool for pet care information using LangChain.
"""
...
def retrieve_pet_care(
    ...
    """
    Retrieves pet care advice from a knowledge base containing authoritative 
    reference sources on pet care information.
    
    Args:
        text: The query to retrieve relevant pet care knowledge.
    ...
    Returns:
        A formatted string containing the retrieved pet care information.
    """
    kb_id = os.environ.get('KNOWLEDGE_BASE_2_ID')
    region_name = os.environ.get('AWS_REGION', 'us-west-2')
    
    if not kb_id:
        return "Error: PET_CARE_KB_ID environment variable not set"
    ...
    try:
        ...
        # Return results
        return f"Retrieved {len(filtered_results)} pet care results with score >= {score}:\n{formatted_results}"

    except Exception as e:
        return f"Error retrieving pet care information: {str(e)}"
